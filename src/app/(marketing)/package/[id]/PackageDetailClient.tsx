...
const PackageContent = ({ initialPackage }: { initialPackage?: any }) => {
  const params = useParams();
  const router = useRouter();
  const rawId = params?.id || params?._id;
  const _id = typeof rawId === 'string' ? rawId : Array.isArray(rawId) ? rawId[0] : (initialPackage?._id || undefined);
  ...
  const { isLoading, error, data } = useQuery({
    ...
    initialData: initialPackage || undefined,
  });
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-1">
            ...
            <div className="flex flex-col sm:items-end gap-1">
              <div className="flex items-center gap-1.5 text-base font-semibold text-gray-900">
                ...
                <span>{data?.gigRating}</span>
                <span className="text-gray-400 font-normal ml-1">({data?.userID?.completedOrdersCount || 0})</span>
              </div>
              ...
              <div className="bg-gray-50 rounded-2xl border border-gray-200/80 p-6 sm:p-8 shadow-2xs">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-sm font-semibold text-amber-500">
                        <span>★</span>
                        <span className="text-gray-800 ml-1">{data?.userID?.starRating} ({data?.userID?.completedOrdersCount || 0})</span>
                      </div>
                    </div>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5 bg-white rounded-xl border border-gray-200/60 text-xs sm:text-sm mb-6 shadow-2xs">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-gray-800">
                      {data?.userID?.responseTimeHours
                        ? `${data.userID.responseTimeHours} ${Number(data.userID.responseTimeHours) === 1 ? 'hour' : 'hours'}`
                        : "1 hour"}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-semibold text-gray-800">
                      {(() => {
                        const deliveryDate = data?.lastDeliveryDate || data?.userID?.lastDeliveryDate;
                        if (!deliveryDate) return "N/A";
                        const date = new Date(deliveryDate);
                        return !isNaN(date.getTime()) ? `${MONTHS[date.getMonth()]} ${date.getFullYear()}` : "N/A";
                      })()}
                    </span>
                  </div>
                </div>
...
export default function PackageDetailClient({ initialPackage }: { initialPackage?: any }) {
  return (
    <Suspense fallback={<div className="flex justify-center items-center h-64"><Loader size={45} /></div>}>
      <PackageContent initialPackage={initialPackage} />
    </Suspense>
  );
}