...
const Review = (props: any) => {
  ...
  // If price/duration aren't returned natively by the review payload anymore, we hide the section, or fall back dynamically if they exist.
  ...
  return (
    <div className="bg-white border border-gray-200/90 rounded-2xl p-6 shadow-2xs hover:border-gray-300 transition-all flex flex-col justify-between h-full">
          ...
          {rating > 0 && (
            <div className="flex items-center gap-1.5 font-bold text-gray-900 text-sm flex-shrink-0">
                ...
                {new Array(rating).fill(0).map((_, i) => (
                    <svg key={`filled-${i}`} className="w-4 h-4 fill-current text-amber-400" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Footer Specs: Price & Duration (Only render if API actually provides these fields) */}
      {(price || duration) && (
