...
const TrustedBy = () => {
  ...
  return (
    <section className="w-full bg-white pb-8 pt-10 sm:pb-12 sm:pt-16 md:pb-14 md:pt-20 lg:pt-24">
      <div className="w-full container mx-auto px-4 md:px-6">
        <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
            {trustFeatures.map((feature, index) => (
              <div key={index} className="flex flex-col items-center text-center px-4 py-8 sm:py-9 lg:py-11">
                <div className="w-[50px] h-[50px] flex items-center justify-center mb-4 text-[#1E293B]">
                  {feature.icon}
                </div>
                <h3 className="font-sf-pro font-semibold text-[24px] text-[#434343] leading-none mb-2 tracking-tight">
                  {feature.title}
                </h3>
                <p className="font-sf-pro font-normal text-[20px] text-[#868686] leading-relaxed max-w-[210px] mx-auto">
                  {feature.description}
                </p>
              </div>
            ))}
